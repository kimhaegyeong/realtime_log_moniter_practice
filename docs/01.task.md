...
- [ ] 프로그래밍 언어 선택 (Go/Python/Node.js)
- [ ] Kafka Producer 라이브러리 설정
- [ ] 로그 생성 로직 구현
...
- [ ] 각각 다른 로그 패턴 생성
- [ ] 부하 테스트 스크립트 작성 (초당 100-1000개 로그 생성)
...
- [ ] MongoDB Aggregation Pipeline 설계
- [ ] 시간대별 통계 집계 (1분, 5분, 1시간 단위)
  - 로그 레벨별 카운트
  - 서비스별 카운트
  - 에러율 계산
- [ ] 집계 결과를 별도 Collection에 저장
- [ ] 스케줄러 구현 (cron 또는 주기적 실행)
- [ ] 집계 API 엔드포인트 개발
  ```
...
- [ ] Grafana 컨테이너 추가 (docker-compose)
- [ ] MongoDB 데이터소스 플러그인 설치
- [ ] 대시보드 패널 구성
  - 실시간 로그 유입량 그래프
  - 서비스별 로그 분포 (Pie Chart)
  - 에러 로그 타임라인
  - 최근 로그 테이블
- [ ] Alert 규칙 설정 (에러율이 임계값 초과 시)
